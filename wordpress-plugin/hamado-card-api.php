...
class Hamado_Card_API {
  ...
  public function routes() {
    ...
    $this->route('/sections', 'GET', 'sections', '__return_true');
    $this->route('/products/by-section/(?P<section>[\w\-]+)', 'GET', 'products_by_section', '__return_true');
  ...
  private function products_list() {
    return [
      ...
      ['id' => 4, 'name' => 'Spotify Subscription', 'icon' => '🎧', 'price' => 3.0],
      ['id' => 5, 'name' => 'TikTok Coins Topup', 'icon' => '🎵', 'price' => 4.0],
      ['id' => 6, 'name' => 'Wallet Payment Topup', 'icon' => '💳', 'price' => 5.0],
    ];
  }
  ...
  private function section_map() {
    return [
      'games' => ['title' => 'الألعاب', 'keywords' => ['free fire', 'pubg', 'game', 'diamond', 'uc']],
      'apps' => ['title' => 'التطبيقات', 'keywords' => ['app', 'application', 'يوهو', 'bigo', 'tiktok', 'instagram']],
      'streaming' => ['title' => 'الاشتراكات', 'keywords' => ['netflix', 'spotify', 'youtube', 'osn', 'subscription']],
      'payments' => ['title' => 'الدفع', 'keywords' => ['wallet', 'payment', 'cash', 'usdt', 'card']],
    ];
  }

  public function sections() {
    return ['success' => true, 'sections' => $this->section_map()];
  }

  public function products_by_section(WP_REST_Request $r) {
    $section = sanitize_text_field((string) $r['section']);
    $map = $this->section_map();
    if (!isset($map[$section])) {
      return ['success' => true, 'section' => $section, 'products' => []];
    }

    $keywords = $map[$section]['keywords'];
    $products = [];
    ...
    return ['success' => true, 'section' => $section, 'products' => $products];
  }
